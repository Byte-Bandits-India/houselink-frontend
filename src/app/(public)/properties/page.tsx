...
function parseUrlParams(
  ...
  let activeTab =
    (searchParams.get("property_purpose") as "sell" | "rent") || "sell";
...
function PropertiesListContent() {
  ...
  // 3. Fetch & Filter Properties whenever Filter Context changes
  useEffect(() => {
    let isCurrent = true;
    setIsLoading(true);
    setError(null);
    ...
    async function fetchFilteredProperties() {
      try {
        ...
        if (filters.priceRanges) {
          const selectedRangeIds = filters.priceRanges
            .split(",")
            .filter(Boolean);
          if (selectedRangeIds.length > 0) {
            const selectedDefs = PRICE_RANGES.filter((r) =>
            ...
            data = data.filter((p) => {
              ...
              return selectedDefs.some(
                (r) =>
                  priceNum >= r.min &&
                  (r.max === Infinity ? true : priceNum <= r.max),
              );
            });
          }
        } else {
          if (filters.minPrice) {
            ...
            if (!isNaN(minLimit))
              data = data.filter((p) => Number(p.price || 0) >= minLimit);
          }
          if (filters.maxPrice) {
