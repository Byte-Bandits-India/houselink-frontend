...
export default async function PropertyDetailPage({
  ...
  const images =
    property.images && property.images.length > 0
      ? property.images.map((img: any) => ({
          image_url: getImageUrl(img.image),
        }))
      : [
          {
            image_url:
              "/assets/images/property_images/1746276498_pexels-kamo11235-667838.jpg",
          },
        ];
  ...
  const similarCategoryLabel =
    SIMILAR_CATEGORY_LABELS[Number(property.categoriesId)] || "Properties";
  const similarLocationLabel =
    property.city?.name ||
  ...
  if (
    property.category?.type?.toLowerCase() !== "commercial" &&
  ...
  if (property.directionFacing)
    allRows.push([
      "Direction Facing",
      formatCapitalize(property.directionFacing),
    ]);
  ...
  if (property.loadingUnloadingFacility != null)
    allRows.push([
      "Loading / Unloading",
      formatBoolean(property.loadingUnloadingFacility),
    ]);
  if (property.petPolicy)
    allRows.push([
      "Pet Policy",
      property.petPolicy === "not_allowed" ? "Not Allowed" : "Allowed",
    ]);
  if (property.foodPreference)
    allRows.push([
      "Food Preference",
      formatCapitalize(property.foodPreference),
    ]);
  if (property.propertySuitableFor)
    allRows.push([
      "Suitable For",
      formatCapitalize(property.propertySuitableFor),
    ]);
          ...
          <div className="flex flex-col xl:flex-row gap-6 h-[90%] w-full">
                ...
                <div>
                  ...
                  {similarCount > 0 && (
                    <span className="inline-flex w-fit items-center gap-1.5 mb-4 px-3 py-1.5 bg-blue-50 text-primary rounded-full text-xs font-bold">
                      {similarCount} {similarCategoryLabel} available in{" "}
                      {similarLocationLabel}
                    </span>
                  )}
                </div>
            ...
            <div className="w-full xl:w-[380px] shrink-0 flex flex-col gap-4">
              ...
              <RequestInfoCard
                ...
                ownerType={formatCapitalize(
                  property.propertyOwnership || "Consultant",
                )}
                property={property}
              />
            </div>
      ...
      <div className="container mx-auto px-4 md:px-6 mt-8 pb-8">
        <div className="flex flex-col xl:flex-row gap-8 items-start">
          ...
          <div className="flex-1 min-w-0 space-y-5">
            ...
            <div
              id="overview"
              className="border border-gray-200 rounded-xl p-5 bg-white scroll-mt-24"
            >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 border border-gray-200 rounded-xl overflow-hidden divide-x divide-gray-200">
                ...
                <OverviewCard
                  icon={
                    <span className="text-gray-500 font-black text-base leading-none select-none">
                      ₹
                    </span>
                  }
                  label="Price"
            ...
            {/* Card: Brokerage */}
            {property.propertyOwnership?.toLowerCase() === "consultant" &&
          ...
          >
            <div className="border border-gray-200 rounded-xl bg-white shadow-xs overflow-hidden">
              <PropertyEnquirySidebar
                property={{ ...property, id: Number(property.id) }}
              />
            </div>
          </div>
        </div>
...
function OverviewCard({
  ...
  return (
    <div className="flex flex-col items-start px-4 py-3 select-none">
      ...
      <p
        className={`font-black text-gray-900 ${small ? "text-sm" : "text-base"} truncate w-full`}
      >
        {value}
      </p>
    </div>
