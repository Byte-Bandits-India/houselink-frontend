...
export default function FAQsPage() {
  ...
  return (
    <div className="bg-surface text-ink overflow-hidden pb-16">
      ...
      <div className="w-full pt-12 pb-6 bg-white border-b border-gray-100 shadow-sm">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
              ...
              <button
                ...
                className={`px-5 py-2 rounded-full text-sm font-semibold transition-all duration-300 border ${
                  selectedCategory === null
                    ...
                    : "bg-white text-[#153e75] border-gray-200 hover:border-[#153e75] hover:bg-gray-50"
                }`}
              >
                All FAQs
              </button>
            ...
            <div className="relative w-full md:w-[320px]">
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-[#153e75]/20 focus:border-[#153e75] transition-all"
              />
      ...
      <div className="container mx-auto px-4 mt-12 max-w-6xl">
        <div className="flex flex-col lg:flex-row gap-10 items-start">
          ...
          {/* ── LEFT SIDEBAR — sticky ── */}
          <aside className="lg:w-[300px] shrink-0 lg:sticky lg:top-8 w-full">

            {/* FAQs label card */}
            <div className="border border-gray-200 rounded-lg px-5 py-4 flex items-center justify-between mb-6 bg-white shadow-sm">
          ...
          <div className="flex-1 min-w-0 w-full">
            ...
            {/* Error State */}
            {error && (
              <div className="flex flex-col items-center justify-center text-center py-10 bg-white border border-red-100 rounded-lg p-6 max-w-xl mx-auto shadow-sm mb-6">
                <AlertCircle size={40} className="text-red-500 mb-3" />
                <h3 className="text-lg font-bold text-gray-800 mb-1">Failed to Load FAQs</h3>
            ...
            {isLoading && !error && (
              <div className="space-y-4">
                {Array.from({ length: 5 }).map((_, idx) => (
                  <div key={idx} className="py-5 border-b border-gray-200 animate-pulse flex flex-col gap-3">
            ...
            {!isLoading && !error && faqs.length === 0 && (
              <div className="flex flex-col items-center justify-center text-center py-16 bg-white border border-gray-100 rounded-lg p-8 max-w-xl mx-auto shadow-sm">
                <BookOpen size={40} className="text-brand mb-3 opacity-40" />
                <h3 className="text-lg font-bold text-gray-800 mb-1">No FAQs Found</h3>
                <p className="text-gray-600 text-sm">
                  We couldn't find any questions matching your filters or search query. Please try searching a different term.
                </p>
              </div>
            )}
            ...
            {!isLoading && !error && faqs.length > 0 && (
              <motion.div
                ...
                viewport={{ once: true }}
                variants={stagger}
                className="border-t border-gray-200 bg-white px-6 rounded-2xl shadow-sm border border-gray-100"
              >
                {faqs.map((item, index) => (
                  <AccordionItem
                    key={item.id}
